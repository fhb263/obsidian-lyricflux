...
export default class DownloadModal extends Modal {
    ...
    private renderResultRow(song: DownloadSong): HTMLElement {
        ...
        // 元数据胶囊行：格式（v1.4.2，非 mp3 才标注）+ VIP 标签 + 时长 + 大小 + 码率（缺项跳过）
    ...
    private sourceRank(src: string): number {
        const order = this.plugin.getSettings().downloadSourceOrder ?? []
    ...
    private appendResultRows(songs: DownloadSong[]): void {
        ...
        for (const s of fresh) {
            // 与 searchCandidates 最终排序同规则：平台优先级主键 → 分高前、同分短名→字典序
