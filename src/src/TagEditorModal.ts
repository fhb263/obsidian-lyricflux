...
import { resolveAudioSource, readMp3Tags, writeMp3Tags, getAudioFileSize, type Mp3Tags, type AudioSource } from 'tags'
...
export default class TagEditorModal extends Modal {
    ...
    private render() {
        ...
        // 「获取歌词」「翻译歌词」放标题行右侧，与封面区按钮同高
        const lyricsSetting = new Setting(contentEl).setName('歌词').setDesc('可整段粘贴 LRC（含 [mm:ss] 时间戳），保存写回 USLT 帧')
    ...
    private async translateLyrics(lyricsArea: HTMLTextAreaElement, btn: ButtonComponent): Promise<void> {
        ...
        // 按当前翻译源取对应 Cookie（百度/有道），其他源无需
        const translateCookie = provider === 'baidu' ? (settings.translateBaiduCookie ?? '')
            : provider === 'youdao' ? (settings.translateYoudaoCookie ?? '')
            : ''
    ...
    private async fetchCover(btn: ButtonComponent): Promise<void> {
        ...
        try {
            const candidates = await this.searchLyricCandidates()
            // 候选带 coverUrl（各平台搜索解析均带）；无 coverUrl 的跳过，取第一个有封面的
            ...
            this.renderCoverPreview()
            this.updateSizeEstimate()
            new Notice(`已导入封面：${withCover.name} - ${withCover.artist}`, 3000)
        } catch (e) {
