...
import {
    ...
    buildBaiduTranslateRequest, parseBaiduTranslateResponse,
    buildYoudaoTranslateUrl, parseYoudaoTranslateResponse, type TranslateProvider,
} from 'translate'
...
export async function searchCandidates(
    ...
    const mapQq = (text: string): DownloadSong[] =>
        parseQqSearchResponse(text).map((s) => ({
            ...
            coverUrl: s.coverUrl, vip: s.vipOnly,
        }))
    ...
    // 按搜索相似度全局排序（而非平台先后）：标题/艺术家命中关键词的分高排前，同分按标题短→字典序
    return out.sort((a, b) => {
...
// --- 在线翻译（v1.4.1）：主 Google、失败降级 MyMemory（编辑标签「翻译歌词」用） ---

/**
 * 翻译一段文本到目标语言（默认中文）。先 Google translate_a/single，失败降级 MyMemory。
 * 两者都失败返回 null（调用方提示网络错误）。
 */
export async function translateText(
    ...
    if (provider === 'baidu') return await translateBaidu(textTrim, target, cookie) ?? await translateMyMemory(textTrim, target)
    if (provider === 'youdao') return await translateYoudao(textTrim, target, cookie) ?? await translateMyMemory(textTrim, target)
    // auto：Google 优先，失败降级 MyMemory
    return await translateGoogle(textTrim, target) ?? await translateMyMemory(textTrim, target)
}

/**
 * 测试翻译源是否可用（设置页翻译 Cookie 行「测试连接」按钮调用）。
 * 用固定测试词「测试」试译到英文；返回 { ok, message }。
 */
...
/** 百度翻译（fanyi.baidu.com/sug 词典接口；未配置 Cookie 视为不可用，降级 MyMemory） */
async function translateBaidu(text: string, target: string, cookie?: string): Promise<string | null> {
...
/** 有道翻译（fanyi.youdao.com/translate；未配置 Cookie 视为不可用，失败降级 MyMemory） */
async function translateYoudao(text: string, target: string, cookie?: string): Promise<string | null> {
    if (!cookie || !cookie.trim()) return null
    try {
 ...
 *  onProgress(done,total) 每批完成后回调供 UI 显示进度；isCancelled 回调返回 true 时立即中断（保留已翻译部分）。
 *  cookie 传给需要登录态的翻译源（当前百度可选）。 */
export async function translateLyricText(
    ...
    for (let i = 0; i < contentLines.length; i += CHUNK) {
        if (isCancelled?.()) break // 中断：保留已翻译部分
        const chunk = contentLines.slice(i, i + CHUNK)
        const results = await Promise.all(chunk.map(async (l) => ({
            time: l.time,
            text: l.text,
            translation: (await translateText(l.text, target, provider, cookie)) ?? undefined,
        })))
